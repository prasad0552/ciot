...
class Settings_ITS4YouProcessFlow_Record_Model extends Settings_Vtiger_Record_Model
{
    ...
    public function getActionsDisplayValue()
    {
        $actions = array();
        /*
        $tasks = Settings_ITS4YouProcessFlow_TaskRecord_Model::getAllForWorkflow($this, true);
    ...
    public function addRelatedAction($RelationData)
    {
        ...
        $adb->pquery("insert into its4you_processflowrel (id, pfid,	parent_id, parent_module, info,	status, for_list, for_detail, for_edit, source_module, if_type) values (?,?,?,?,?,?,?,?,?,?,?)", array($relId, $pfid, $parent_id, $parent_module, $info, "0", $for_list, $for_detail, $for_edit, $source_module_name, $if_type));
    ...
    public function getQueryResultRows($mode = "detail", $type = "", $if_type = "")
    {
        ...
        $query = 'SELECT its4you_processflowrel.*, \'getProcessFlowActionData\' AS method_name FROM its4you_processflowrel WHERE deleted = ? AND pfid = ? ';
        $attr = array('0', $pfid);
        if ($mode == "execute") {
            $query .= ' AND status = ? ';
            $attr[] = '0';
