...
class Settings_FieldDependency_ListView_Model extends Settings_Vtiger_ListView_Model
{
    ...
    public function getListViewEntries($pagingModel)
    {
        ...
        $dependentFields = Vtiger_DependencyField::getDependentFields($forModule);

        $noOfRecords = count($dependentFields);
        $recordModelClass = Vtiger_Loader::getComponentClassName('Model', 'Record', 'Settings:FieldDependency');

        $listViewRecordModels = array();
        for ($i = 0; $i < $noOfRecords; $i++) {
            $record = new $recordModelClass();
            $module = $dependentFields[$i]['module'];
            unset($dependentFields[$i]['module']);
            $record->setData($dependentFields[$i]);
