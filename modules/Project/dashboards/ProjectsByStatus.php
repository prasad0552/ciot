...
class Project_ProjectsByStatus_Dashboard extends Vtiger_IndexAjax_View
{
    function getSearchParams($value, $assignedto, $dates)
    {
        ...
        $conditions = array(array('projectstatus', 'e', decode_html(urlencode(escapeSlashes($value)))));
        if ($value == vtranslate('LBL_BLANK', 'Activities')) {
            $conditions = array(array('projectstatus', 'y'));
        }
    ...
    public function process(Vtiger_Request $request)
    {
        ...
        $data = $moduleModel->getProjectsByStatus($request->get('smownerid'), $dates);
