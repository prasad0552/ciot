...
class Vtiger_DependencyField
{
    static function getDependentFields($module = '')
    {
        global $adb;
        ...
        if ($noofrows > 0) {
            ...
            for ($i = 0; $i < $noofrows; ++$i) {
                ...
                $dependentFields[] = array('sourcefield' => $sourceField, 'sourcefieldlabel' => $sourceFieldLabel,
    ...
    static function getFieldDependencyDatasource($module)
    {
        global $adb;

        $tabId = getTabid($module);
        $fieldDependencyDatasource = array();
        $moduleModel = Vtiger_Module_Model::getInstance($module);
        $fieldDependencyDatasource = $moduleModel->getCustomPicklistDependency();

        $result = $adb->pquery('SELECT sourcefield,targetfield,sourcevalue,targetvalues,criteria FROM vtiger_field_dependency WHERE tabid=?', array($tabId));
        $noofrows = $adb->num_rows($result);
        ...
        for ($i = 0; $i < $noofrows; ++$i) {
            ...
            if (!empty($unserializedCriteria) && $unserializedCriteria['fieldname'] != null) {
                ...
                $fieldDependencyDatasource[$sourceField][$sourceValue][$targetField][] = $conditionValue;
            } else {
                $fieldDependencyDatasource[$sourceField][$sourceValue][$targetField] = $unserializedTargetValues;
            }
            $fieldDependencyDatasource[$sourceField]['__DEFAULT__'][$targetField] = $pickArray;
        }
        return $fieldDependencyDatasource;
    }
