...
class Vtiger_Field_Model extends Vtiger_Field
{
     ...
     * @return <Boolean> - true/false
     */
    public function isViewableInEditView()
    {
        if (!$this->isEditEnabled()
            || !$this->isViewable()
            || !in_array(((int)$this->get('displaytype')), array(1, 5))
            || $this->isReadOnly() == true
            || $this->get('uitype') == 4
            || in_array($this->get('editview'), array(1, 3))
        ) {
            return false;
        }
        return true;
    ...
    /**
     * Function to check whether the current field is enabled in create view
     * @return <Boolean> - true/false
     */
    public function isViewableInCreateView()
    {
        if (!$this->isEditEnabled()
            || !$this->isViewable()
            || !in_array(((int)$this->get('displaytype')), array(1, 5))
            || $this->isReadOnly() == true
            || $this->get('uitype') == 4
            || in_array($this->get('createview'), array(1, 3))
        ) {
            return false;
        }
        return true;
    ...
    {
        $db = PearDatabase::getInstance();
        $query = 'UPDATE vtiger_field SET typeofdata=?,presence=?,quickcreate=?,createview=?,editview=?,masseditable=?,defaultvalue=?,summaryfield=?,headerfield=?';
        $params = array($this->get('typeofdata'), $this->get('presence'), $this->get('quickcreate'), $this->get('createview'), $this->get('editview'),
