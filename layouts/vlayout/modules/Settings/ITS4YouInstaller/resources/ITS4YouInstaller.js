...
jQuery.Class("Settings_ITS4YouInstaller_Extensions_Js", {
    ...
    registerEventsForInstaller: function(container) {
        ...
        jQuery(container).off('click').on('click', '.oneclickInstallFree', function(e) {
            let element = jQuery(e.currentTarget);
            let extensionContainer = element.closest('.extension_container');
            let extensionId = extensionContainer.find('[name="extensionId"]').val();
            let moduleAction = extensionContainer.find('[name="moduleAction"]').val();
            let extensionName = extensionContainer.find('[name="extensionName"]').val();
