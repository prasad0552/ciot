...
{*        {/if}*}
        ...
        {if $QUICK_PREVIEW_ENABLED eq 'true'}
            <span>
			<a class="quickView fa fa-eye icon action" data-app="{$SELECTED_MENU_CATEGORY}"
               title="{vtranslate('LBL_QUICK_VIEW', $MODULE)}"></a>
		</span>
        {/if}
        {if $MODULE_MODEL->isStarredEnabled()}
            <span>
			<a class="markStar fa icon action {if $STARRED} fa-star active {else} fa-star-o{/if}"
...
{*                <span><a class="fa icon action fa-edit" data-id="{$LISTVIEW_ENTRY->getId()}" href="javascript:void(0);"*}
{*                         data-url="{$LISTVIEW_ENTRY->getEditViewUrl()}&app={$SELECTED_MENU_CATEGORY}" name="editlink"*}
{*                         title="{vtranslate('LBL_EDIT', $MODULE)}"></a></span>*}
{*            {/if}*}
{*            {if $RECORD_ACTIONS['delete']}*}
{*                <span><a class="fa icon action fa-remove deleteRecordButton" data-id="{$LISTVIEW_ENTRY->getId()}"*}
{*                         href="javascript:void(0);" title="{vtranslate('LBL_DELETE', $MODULE)}"></a></span>*}
{*            {/if}*}
{*        {/if}*}
...
{*                    <li><a data-id="{$LISTVIEW_ENTRY->getId()}" href="{$LISTVIEW_ENTRY->getFullDetailViewUrl()}&app={$SELECTED_MENU_CATEGORY}">{vtranslate('LBL_DETAILS', $MODULE)}</a></li>*}
        ...
        <div class="btn-group inline-save hide">
            <button class="button btn-success btn-small save" type="button" name="save"><i class="fa fa-check"></i>
            </button>
            <button class="button btn-danger btn-small cancel" type="button" name="Cancel"><i class="fa fa-close"></i>
            </button>
        </div>
    </div>
{/strip}
