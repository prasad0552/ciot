...
<div class="container-fluid" id="moduleManagerContents">
    ...
    <div class="pcss3t pcss3t-icons-bottom pcss3t-theme-5 pcss3t-height-auto">
        <input type="radio" name="pcss3t" checked  id="tab1" class="tab-content-first" {if $current_tab eq 'tab1'}checked="checked"{/if}>
        <label for="tab1">General Settings<i class="fa fa-cogs" aria-hidden="true"></i></label>

        <input type="radio" name="pcss3t"  id="tab2" class="tab-content-2" {if $current_tab eq 'tab2'}checked="checked"{/if}>
        <label for="tab2">Partial DetailView<i class="fa fa-columns" aria-hidden="true"></i></label>

        {*<input type="radio" name="pcss3t"  id="tab3" class="tab-content-3" {if $current_tab eq 'tab3'}checked="checked"{/if}>*}
        {*<label for="tab3">Related Tabs<i class="fa fa-columns" aria-hidden="true"></i></label>*}

        {*<input type="radio" name="pcss3t"  id="tab4" class="tab-content-4" {if $current_tab eq 'tab4'}checked="checked"{/if}>*}
        {*<label for="tab4">E-Mail Log<i class="fa fa-columns" aria-hidden="true"></i></label>*}

        {*<input type="radio" name="pcss3t"  id="tab5" class="tab-content-5" {if $current_tab eq 'tab5'}checked="checked"{/if}>*}
        {*<label for="tab5">ListView Quicksearch<i class="fa fa-columns" aria-hidden="true"></i></label>*}
                        ...
                        <table style="width:100%;">
                            ...
                            <tr>
                                <td colspan="2">
                                    ...
                                    <button class="btn btn-default" type="button" onclick="return SwVtTools.GeneralOptions('checkModuleFields', {ldelim}moduleName:$('#checkColumnsModule').val(){rdelim}, true);">Check missing table columns for fields in Database</button>
                                </td>
                            </tr>
                            <tr>
                ...
                <form method="POST" action="#">
                    ...
                    <table class="table table-condensed">
                        {foreach from=$relTabs item=tab}
                            <tr>
                                <td style="width:200px;"><i class="fa fa-trash trashRelTabOrder" data-id="{$tab.id}" aria-hidden="true"></i>&nbsp;&nbsp;{vtranslate($tab.modulename, $tab.modulename)}</td>
                                <td>
            ...
            <li class="tab-content tab-content-5">
                {if !empty($listwidgets)}
                    <form method="POST" action="index.php?module=SwVtTools&view=Index&parent=Settings&tab=tab5">
                        ...
                        <table class="table table-condensed">
                        {foreach from=$listwidgets item=widget}
                            <tr>
                                <td><input type="checkbox" name="widget[{$widget.id}][active]" value="1" {if $widget.active eq '1'}checked="checked"{/if} /></td>
                                <td><input type="hidden" name="widget[{$widget.id}][module]" value="{$widget.module}" />{$widget.module}</td>
                                <td><input type="text" name="widget[{$widget.id}][title]" value="{$widget.title}" /></td>
                                <td><select class="select2" style="width:300px;" name="widget[{$widget.id}][fields][]" multiple>
                                        {foreach from=$moduleFields[$widget.module] key=blockLabel item=blockFields}
                                            <optgroup label="{$blockLabel}">
                                                {foreach from=$blockFields item=field}
                                                    <option value="{$field->name}" {if !empty($widget.settings.fields) && in_array($field->name, $widget.settings.fields)}selected="selected"{/if}>{$field->label}</option>
                                                {/foreach}
                                            </optgroup>
                                        {/foreach}
                                    </select></td>
